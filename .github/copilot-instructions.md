...
| After a big feature sweep: verify every change is wired end-to-end (pages reachable, routes called, exports/types consumed, env documented) with no orphaned seams, then run the gates → seam report + go/no-go | **Integration Sweep Agent** | read-only + runs gate |
...
| Deep, repeatable UI/UX/security/usability/supportability review with stable check IDs, computed severity, and run-over-run diffing (NEW/REGRESSED/FIXED/PERSISTENT) → machine-readable manifest + scored report (`reports/deep-review.{md,json}`) | **Deep Review Harness** | read-only + runs gate |
| Deep, repeatable PRODUCT review — product clarity, feature coverage vs the promise, end-to-end coach flows, field usability, content/tone, MVP scope (V1/V2/V3); "is it feature-complete, fast, field-ready, and better than a spreadsheet?" → scored 13-section report + manifest (`reports/product-review.{md,json}`) | **Product & UX Review Harness** | read-only + runs gate |
...
| Compare against the coaching-tools / game-day competitors, update the parity matrix | **Competitor Parity Tracker** | read-only |

Prefer the read-only auditors (Reviewer, Guardian, Auditor, Parity Tracker, Security Review Agent) for "review/audit/what's wrong" asks, then hand fixes to Code Optimizer or Corpus Curator. The **Integration Sweep Agent** is the first stop after a large feature sweep — it catches orphaned seams (unreachable pages, uncalled routes, unused exports, undocumented env) before the deeper gates run. The **Security Review Agent** is the stricter, launch-blocking gate for anything touching auth, data isolation, secrets, billing, or child/player privacy. The **Deep Review Harness** is the deepest manual pass on *engineering quality* — a repeatable UI/UX/security/usability/supportability sweep with stable check IDs and run-over-run diffing; reach for it when you want trendable evidence across runs, not a one-time audit. The **Product & UX Review Harness** is its product-side sibling — the same repeatable, diffable contract aimed at *product completeness and field usability* (feature coverage vs the promise, end-to-end coach flows, mobile field-use, content/tone, and MVP scope V1/V2/V3); reach for it to answer "is the app feature-complete, fast, field-ready, and better than a spreadsheet?"
